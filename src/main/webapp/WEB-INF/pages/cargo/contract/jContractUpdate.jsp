		...
		<table class="commonTable" cellspacing="1">
	        ...
	        <tr>
	            ...
	            <td class="tableContent">
	            	<input class="input" type="radio" name="printStyle" value="2" <c:if test="${obj.printStyle=='2'}">checked</c:if>/>两款
	            	<input class="input" type="radio" name="printStyle" value="1" <c:if test="${obj.printStyle=='1'}">checked</c:if>/>一款
	            </td>
	        </tr>
	        <tr>
	            <td class="columnTitle_mustbe">签单日期：</td>
	            <td class="tableContent">
	            	<input type="text" style="width: 90px" name="signingDate" value="<fmt:formatDate pattern="yyyy-MM-dd" value="${obj.signingDate}"/>" 
	            		onClick="WdatePicker({el:this,isShowOthers:true,dateFmt:'yyyy-MM-dd'});"/>
	            </td>
	            <td class="columnTitle_mustbe">重要程度：</td>
	            <td class="tableContent">
	            	<input class="input" type="radio" name="importNum" value="3" <c:if test="${obj.importNum==3}">checked</c:if>/>★★★
	            	<input class="input" type="radio" name="importNum" value="2" <c:if test="${obj.importNum==2}">checked</c:if>/>★★
	            	<input class="input" type="radio" name="importNum" value="1" <c:if test="${obj.importNum==1}">checked</c:if>/>★
	            </td>
	        </tr>
	        <tr>
	            <td class="columnTitle_mustbe">交货期：</td> 
	            <td class="tableContent">										
	            <input type="text" style="width: 90px" name="deliveryPeriod" value="<fmt:formatDate pattern="yyyy-MM-dd" value="${obj.deliveryPeriod}"/>"
	            	onClick="WdatePicker({el:this,isShowOthers:true,dateFmt:'yyyy-MM-dd'});"/>
	            </td>
	            <td class="columnTitle_mustbe">船期：</td>
	            <td class="tableContent">												
					 <input type="text" style="width: 90px" name="shipTime" value="<fmt:formatDate pattern="yyyy-MM-dd" value="${obj.shipTime}"/>"
	            		onClick="WdatePicker({el:this,isShowOthers:true,dateFmt:'yyyy-MM-dd'});"/>
				</td>
	        </tr>
	        <tr>
